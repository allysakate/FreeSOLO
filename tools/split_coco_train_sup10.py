...
def split_json():
    ...
    save_path_labeled = os.path.join(
        dataset_dir, "instances_train2017_sup5_seed1_labeled.json"
    )
    save_path_unlabeled = os.path.join(
        dataset_dir, "instances_train2017_sup5_seed1_unlabeled.json"
    )
    ...
    print(
        "Labeled data: {} images, {} annotations.".format(
            len(labeled_images), len(selected_anns)
        )
    )
